...
The pure task logic (schema/model/catalog/labels/extensions) has NO third-party
dependency and is imported eagerly. The pandas-dependent submodules (adapters,
fidelity) are imported lazily on first attribute access, so the label functions remain
usable where pandas/pm4py/ocpa are absent.
  ...
  extensions  object-enabled extension tasks (X-Inf, X-MSt); NOT in the main catalog
...
"""
from . import schema, model, catalog, labels, extensions  # noqa: F401  (pure, no pandas)

__all__ = ["schema", "model", "catalog", "labels", "extensions", "fidelity", "adapters"]
