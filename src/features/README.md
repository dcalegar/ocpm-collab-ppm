...
by RQ3. Decoupled from [`ocpm_eval`](../ocpm_eval/README.md): it depends only
on [`ocpm_tasks`](../ocpm_tasks/README.md) (for the neutral model and `Schema`) and
...
Consumed by `ocpm_eval.rq3_pipeline`, which pairs
`feats["table"]` with a `predictors.dispatch` predictor and a `GroupKFold` split.
