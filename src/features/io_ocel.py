...
``load_ocpa_ocel`` is also imported by features.ocpa and rq3_pipeline so the OCEL object
...
# instead of 'in_participant'. The core mapping (M6) never emits 'from'/'to'
# as an E2O qualifier itself (only in_collaboration/in_orchestration/
# in_participant/send/receive), so every E2O row under these two
...
# reached by both 'in_participant' and a 'from'/'to' witness at once. Both
...
def _cross_case_qualifiers(schema) -> Tuple[str, ...]:
    ...
    collaboration cases -- the direct in_participant edge (M6) and the D25
    witness edges that reuse 'from'/'to' at the E2O level."""
    return (schema.q_in_participant, schema.q_from, schema.q_to)
...
def _strip_participant_e2o(path: str, schema) -> str:
    ...
    shared, log-wide object removed -- the direct 'in_participant' edge (M6)
